...
class MSM_observer_Michelangelo_GP:
    def __init__(self, device, kwargs):
        ...
        self.Gamma2 = reshape_pt1(torch.as_tensor([
...
class MSM_justvelocity_observer_adaptive_highgain_GP:
    ...
    def call_noadapt(self, t, xhat, u, y, t0, init_control, GP, kwargs,
        ...
        K = torch.zeros(self.n, device=self.device)
        for i in range(self.n):
            K[i] = kwargs.get('prior_kwargs').get('observer_gains').get(
                'k' + str(i + 1))
        Gamma1 = self.K * torch.tensor([
            g ** (i + 1) for i in range(self.n)], device=self.device)
        return MSM_justvelocity_observer_highgain_GP.__call__(
            self, t=t, xhat=xhat, u=u, y=y, t0=t0, init_control=init_control,
            GP=GP, kwargs=kwargs, impose_init_control=impose_init_control,
            Gamma1=Gamma1)
