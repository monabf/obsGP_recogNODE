...
from model_evaluation.plotting_functions import save_outside_data, \
    plot_outside_data
from utils.utils import reshape_pt1, reshape_dim1, interpolate_func, start_log, \
    stop_log
...
# elbow pipe (as simulated by Fluent) with a GP. Measurement of all modes are
# assumed so that their dynamics can be learned directly.
...
if __name__ == '__main__':
    ...
    config = Config(system='Continuous/Fluent_elbow/Discrete_model/'
                           'No_observer_noisy_inputs',
                    data_folder='../Data/My_wholefield3_Fluent_elbow_testcase/',
                    rom_folder='../Data/My_wholefield3_Fluent_elbow_testcase/roms/good7_learn12_5modes_noSteadyInit_firstOrder/',
                    use_GPy=True,
                    ...
                    t0=0,
                    tf=10000,
                    hyperparam_optim='fixed_all_parameters',
                    true_meas_noise_var=1e-3,
                    process_noise_var=0,
                    meas_noise_var=1e-6,  # TODO
                    simu_solver='dopri5',
                    nb_rollouts=1,
                    rollout_length=10,
    ...
    if config.GP_optim_stopper:
        config.update(dict(
            optim_stopper_patience=config.GP_optim_stopper.patience,
            optim_stopper_threshold=config.GP_optim_stopper.threshold))
    ...
    if 'Continuous/Fluent_elbow' in config.system:
        ...
        if 'No_observer' in config.system:
            config.update(dict(
        ...
        # hyperparameter optimization, and an initial value. For more
        # diversity in the hyperoptim results, initialize from a normal with
        # mean = hyperpriors, then run LBFGS with large step size and
        # strong_wolfe
        if config.sparse and (config.sparse.get('method') == 'SparseGP'):
            ...
            likelihood.register_prior(
                ...
                gpytorch.priors.MultivariateNormalPrior(torch.tensor(
                    [config.meas_noise_var] * nb_SVD),
                    torch.diag(torch.tensor([config.meas_noise_var] * nb_SVD))),
                'noise')
        ...
        if config.prior_mean == read_discrete_GP_prior:
            hypers = {'base_kernel.lengthscale':
                          torch.tensor([150., 80., 90., 90., 90., 150., 150.]),
                      'outputscale': torch.tensor([10.])}  # dx=3
            prior_kernel = gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.RBFKernel(
                    batch_shape=torch.Size([config.nb_SVD_modes]),
                    ard_num_dims=input_dim),
                batch_shape=torch.Size([config.nb_SVD_modes]))
            ...
            config.update(dict(prior_mean=read_prior_GP.predict_onlymean))
    ...
    for fold_nb in range(config.nb_loops):
        ...
        if ('noisy_inputs' in config.system) and \
                (config.true_meas_noise_var != 0):
            if not torch.is_tensor(config.get('true_vector_meas_noise_var')):
                config.update(dict(  # covar output = C covar C.T
                    true_vector_meas_noise_var=config.true_meas_noise_var *
                                               torch.max(torch.abs(xtraj),
                                                         dim=0).values))
                config.update(dict(true_meas_noise_var=torch.matmul(
                    torch.matmul(
                        config.prior_kwargs['original_observation_matrix'],
                        torch.diag(config.true_vector_meas_noise_var)),
                    config.prior_kwargs['original_observation_matrix'].t())))
            distrib = \
                torch.distributions.multivariate_normal.MultivariateNormal(
                    torch.zeros(xtraj.shape[1]), torch.diag(
                        config.true_vector_meas_noise_var))
            xtraj = xtraj + distrib.sample((len(xtraj),))
