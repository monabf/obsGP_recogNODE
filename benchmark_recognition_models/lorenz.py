...
# NODE settings and observation data) for the Lorenz model with known dynamics
...
if __name__ == '__main__':
    ...
    # General params
    config = Config(system='Continuous/Benchmark_recognition/Lorenz'
                           '/MLP2_noisy_inputs',
                    ...
                    nb_rollouts=10,  # TODO
                    rollout_length=100,
                    rollout_controller={'null_controller': 10},
                    rollout_controller_args={'controller_args': [{}] * 10},
    ...
    if train:
        ...
        submodel = Submodel(config.sigma, config.rho, config.beta)
        ...
        if config.init_state_obs_method.startswith('y0T_u0T_RNN_outNN'):
            ...
            init_state_model = MLPn_xin(
                n_in=dz, num_hl=2, n_hl=50, n_out=config.n,
            ...
            init_state_model = MLPn(num_hl=2,
        ...
        # tensorboard --logdir=../Figures/Continuous/Lorenz/MLP2_noisy_inputs/ --port=8080
