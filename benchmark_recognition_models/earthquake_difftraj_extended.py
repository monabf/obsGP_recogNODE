...
from simulation.dynamics_functions import Earthquake_building_extended
...
from benchmark_NN_models import MLP2, Earthquake_extended_recog
...
# earthquake, extended to have this sinusoidal disturbance as an extra state,
# i.e. add a harmonic oscillator in the model (linear and autonomous,
# observable!)
...
if __name__ == '__main__':
    ...
    config = Config(system='Continuous/Benchmark_recognition/'
                           'Earthquake_building_extended/MLP2_noisy_inputs',
    ...
    if train:
        ...
        config.update(dict(
            ...
            dynamics_function=Earthquake_building_extended,
            no_control=False,
        ...
        # Initialize the extended states randomly using constant control
        init_state_x[:, :, -1] = torch.tensor([0.])
        init_state_x[:, :, -2] = torch.tensor([[
            config.controller_args[i]['control_value'][0]] for i in range(
            len(config.controller_args))])
        ...
        init_state_model1 = MLP2(
            ...
            n_h2=50, n_out=config.n - 2, activation=nn.Tanh)  # TODO
        init_state_model = Earthquake_extended_recog(
            init_state_model1, config=config, n2=1, n3=1, idx11=0,
            idx12=torch.numel(diff_init_state_obs[0]) - config.nu,
            idx21=-config.nu, idx22=-config.nu + 1,
            idx31=-config.nu + 1, idx32=None)
