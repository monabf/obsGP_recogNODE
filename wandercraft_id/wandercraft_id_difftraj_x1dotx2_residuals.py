...
from wandercraft_id_NN_models import MLPn, WDC_simple_recog4, \
    WDC_single_deformation_model, WDC_x1dotx2_resmodel, RNNn, MLPn_xin
...
if __name__ == '__main__':
    ...
    config = Config(system='Continuous/Wandercraft_id/MLPn_noisy_inputs/'
                    ...
                    NODE_file='../Figures/Continuous/Wandercraft_id/'
                              'MLPn_noisy_inputs/x1dotx2_residuals/7-29-2difftraj_0.001subsampling_200samples_filter_gaussian50_butteru/'
                              'KKL_u0T_back/0.9199040056183782_autograd_200samples_noise0.0_NODE_difftraj265_Adam0.005_KKL_u0T_back100/Learn_NODE.pkl',
    ...
    f = MLPn(num_hl=5, n_in=config.n + nu_submodel, n_hl=100, n_out=2,
             activation=nn.SiLU())
    submodel = WDC_x1dotx2_resmodel(f, config)  # TODO
    ...
    if 'KKL_u0T' in config.init_state_obs_method:
        ...
        D = torch.tensor([[-16.1729, -6.5789, -6.7711, -6.4036, -6.0087,
                           -8.9313, -6.3548,
                           -5.7078, -6.1041, -7.6046],
                          [-3.1614, -23.5227, -3.0865, -3.1089, -2.6219,
                           -2.7422, -2.9107,
                           -2.7944, -2.9218, -3.0121],
                          [-1.6484, -2.2043, -32.9249, -2.4694, -1.8331,
                           0.8310, -2.1117,
                           -0.6051, -1.9164, -1.4850],
                          [-3.5210, -3.1154, -2.7836, -42.7794, -0.5091,
                           4.8344, -2.1954,
                           0.6314, -1.9838, -0.8516],
                          [-3.0534, -2.8723, -3.5935, -3.7212, -54.9133,
                           -0.6485, -3.4684,
                           -4.1717, -3.7255, -2.5881],
                          [2.4535, 1.8759, 1.5912, 1.0531, 0.4074, -61.5493,
                           1.1621,
                           0.4740, 1.3044, 2.6631],
                          [-2.3621, -2.6461, -2.5159, -2.1873, -0.9764,
                           0.6992, -71.9472,
                           -0.2756, -1.6963, -2.0933],
                          [-5.0060, -3.7195, -3.0739, -3.6190, -2.6876,
                           5.4839, -3.0791,
                           -82.8536, -3.2527, -0.8659],
                          [-2.8983, -2.9195, -2.6662, -2.6445, -0.9672,
                           2.7929, -2.0922,
                           -0.4813, -92.0617, -1.3742],
                          [6.5495, 5.9176, 5.6541, 5.6170, 5.7791, 4.3676,
                           5.4569,
                           5.1445, 5.0568, -94.9428]])
        ...
        # data = pd.read_csv(
        #     '../Data/Wandercraft_id/KKLu_back_D1_x1dotx2_residuals.csv',
        #     sep=',', header=None)
    ...
    if train:
        ...
        #
        # file = '/Users/mona/PhD_code/observer_add_GPyTorch_advanced/Figures' \
        #        '/Continuous/Wandercraft_id/MLPn_noisy_inputs' \
        #        '/x1dotx2_residuals/7-29-2difftraj_0' \
        #        '.001subsampling_200samples_filter_gaussian50_butteru' \
        #        '/KKLu_back_optimD/Notgreat1_diag2_dz50_autograd_200samples_noise0.0_NODE_difftraj265_Adam0.005_KKLu_back_optimD100'
        # saving = '/Users/mona/PhD_code/observer_add_GPyTorch_advanced/Data/Wandercraft_id'
        # NODE = pkl.load(open(file + '/Learn_NODE.pkl', 'rb'))
        # df = pd.DataFrame(NODE.z_config['D'].numpy())
        # df.to_csv(os.path.join(saving, 'KKLu_back_D1_x1dotx2_residuals.csv'),
        #           header=False)
        # wait = scikit-learn
    ...
    if test:
        ...
        # random_idx = torch.tensor([0, 10, 31])
        # X_test = X_test[random_idx, :2000]
        # U_test = U_test[random_idx, :2000]
        # X_test = X_test[25].reshape(1, X_test.shape[1], X_test.shape[2])
        # U_test = U_test[25].reshape(1, U_test.shape[1], U_test.shape[2])
