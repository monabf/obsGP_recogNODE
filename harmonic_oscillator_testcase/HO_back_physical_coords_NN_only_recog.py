...
if __name__ == '__main__':
    ...
    if train:
        ...
        if 'Continuous/Harmonic_Oscillator' in config.system:
            ...
            config.update(dict(
                dynamics=dynamics.harmonic_oscillator_extended_dynamics,
                dynamics_x=dynamics.harmonic_oscillator_extended_dynamics,
                true_dynamics=dynamics.harmonic_oscillator_extended_dynamics))
            print(config.pulse)
            ...
            init_state_x = torch.cat((
                torch.unsqueeze(torch.as_tensor(sampling(
                    config.nb_difftraj)), 1),
                config.pulse ** 2 * torch.ones(config.nb_difftraj, 1, 1)),
                dim=2)
            # xlimits = np.array([[-1., 1.], [-1., 1.], [0.8, 1.]])
            # sampling = LHS(xlimits=xlimits)
            ...
            submodel = Submodel(config.pulse)
            ...
            config.update(dict(
                ...
                constrain_u=[-1., 1.],
                constrain_x=[],
                grid_inf=[-1, -1, config.pulse ** 2],  # , 0.8],  # TODO
                grid_sup=[1, 1, config.pulse ** 2]))  # , 1.]))
        ...
        if 'KKL_u0T' in config.init_state_obs_method:
            ...
            if 'back' in config.init_state_obs_method:
                D = torch.tensor([[-5.7468, -3.6970, 2.2523, -1.0514],
                                  [3.2455, -4.7005, -1.0364, 1.0700],
                                  [2.6665, 1.4189, -1.9953, -3.9923],
                                  [-1.0521, 0.4193, 5.4627, -2.0062]])
                F = torch.ones(dz, y_observed_true.shape[1])
            else:
                D = torch.tensor([[-4.3676, -3.7728, 2.7256, -0.3862],
                                  [-1.0081, -4.8401, -2.8531, 0.2585],
                                  [0.6691, 1.1287, -2.5718, -4.5086],
                                  [-0.3124, 0.1549, 4.6348, -2.3672]])
                F = torch.ones(dz, y_observed_true.shape[1])
            z0 = torch.zeros(1, dz)
