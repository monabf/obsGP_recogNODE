...
# Impose x1dot = x2 and x2dot = - grad(H(x1))
    ...
    if train:
        ...
        if 'Continuous/Harmonic_Oscillator' in config.system:
            ...
            class Submodel(nn.Module):
                def __init__(self, gradHamiltonian):
                    super(Submodel, self).__init__()
                    self.gradH = gradHamiltonian
                ...
                def forward(self, x):
                    # Not learning whole Hamiltonian, only x2dot = -gradH(x1)
                    # xx1dot = x2 known
                    xdot = torch.zeros_like(x)  # shape (N, 1, n)
                    xdot[..., 0] = x[..., 1]
                    xdot[..., 1] = -self.gradH(
                        reshape_dim1(x[..., 0])).view(xdot[..., 1].shape)
                    return xdot


            H = MLP2(n_in=config.n + nu_submodel - 1, n_h1=25, n_h2=25, n_out=1,
                     activation=nn.SiLU)
            config.update(dict(gradHamiltonian=H))
            submodel = Submodel(H)
