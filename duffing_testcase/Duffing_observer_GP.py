...
if __name__ == '__main__':
    ...
    config = Config(system='Continuous/Duffing/Discrete_model/'
                    ...
                    meas_noise_var=0.1,
                    simu_solver='dopri5',
                    solver_options={'rtol': 1e-8, 'atol': 1e-8},  # for back
                    nb_loops=10,
                    nb_rollouts=10,
                    rollout_length=300,
    ...
    if 'Continuous/Duffing' in config.system:
        ...
        # init_state = torch.tensor([[0., -1.]])  # TODO
        # # Random initial state
        xmin = torch.tensor([-1., -1.])
        xmax = torch.tensor([1., 1.])
        init_state = reshape_pt1(torch.rand((1, 2)) * (xmax - xmin) + xmin)
        init_state_estim = torch.cat((
            reshape_pt1(init_state[0, 0]),
        ...
        if 'GP_Michelangelo' in config.system:
            config.update(dict(
                ...
                # add_u_obs_justvelocity=True,
                prior_kwargs={'alpha': 0, 'beta': 0, 'delta': 0, 'gamma': 0.4,
                              ...
                              'observer_gains': {'g': 8.,
                                                 ...
                                                 'k2': 5.,
                                                 'k3': 1.}},
                              # 'observer_gains': {'g': 11.,  # TODO comparison
                              #                    'k1': 1.52862409,
                              #                    'k2': 0.9736215,
                              #                    'k3': 0.24805021}},
                saturation=np.array([-30, -1]),
                init_state_estim=torch.cat((
                    config.init_state_estim, torch.tensor([[0.]])), dim=1)))
            prior = Duffing_prior(config.cuda_device, config.prior_kwargs)
            config.update(dict(
                observer_prior_mean=prior.continuous_Michelangelo_deriv,
                prior_mean=prior.continuous_Michelangelo_u,
                prior_mean_deriv=prior.continuous_Michelangelo_deriv_u))
            ...
            if config.continuous_model:
                ...
                config.update(dict(
                    ...
                    observer_prior_mean=prior.continuous_justvelocity,
                    prior_mean=prior.continuous_justvelocity,
                    prior_mean_deriv=None))
            else:
                prior = Duffing_prior(config.cuda_device, config.prior_kwargs)
                config.update(dict(
                    saturation=np.array(
                        [-5 * config.get('prior_kwargs').get('delta')]),
                    observer_prior_mean=prior.continuous_justvelocity,
            ...
            config.update(dict(
                ...
                prior_kwargs={'alpha': 0, 'beta': 0, 'delta': 0, 'gamma': 0.4,
                              ...
                              'dt_before_subsampling': 0.001,
                              'observer_gains':
                                  {'g': 15, 'g0': 15, 'k1': 5, 'k2': 5,
                                   'p1': 300, 'p2': 1e-5, 'b': 1e-4,
                                   'n': init_state.shape[1],
                                   'adaptation_law':
                                       Praly_highgain_adaptation_law}},
                saturation=np.array(
                    [-5 * config.get('prior_kwargs').get('delta')])))
        ...
        lengthscale_prior = gpytorch.priors.MultivariateNormalPrior(
            torch.tensor([5., 15., 150.]),
            torch.diag(torch.tensor([100., 100., 100.])))
        outputscale_prior = gpytorch.priors.NormalPrior(110., 100.)
        ...
        if config.use_GPy:
            kernel = GPy.kern.RBF(input_dim=input_dim, variance=110.,
                                  lengthscale=np.array([5., 15., 150.]),
                                  ARD=True)
            kernel.unconstrain()
            kernel.variance.set_prior(GPy.priors.Gaussian(110, 100.))
    ...
    def true_dynamics(x, control):
        ...
        if ('justvelocity' in config.system) and (
                'Duffing' in config.system):
            if config.continuous_model:
                xnext = config.dynamics_function(
                    t=t0_init, x=x, u=true_controller, t0=t0_init,
            ...
            xnext = torch.tensor(
                - config.get('beta') * x[:, 0] ** 3 - config.get(
                    'alpha') * x[:, 0] - config.get('delta') * x[:, 1],
                device=device)
        elif (('Michelangelo' in config.system) or (
                'justvelocity_highgain' in config.system)) \
                and not any(
            k in config.system for k in ('Duffing')):
    ...
    for loop in range(config.nb_loops):
        ...
        if config.verbose:
            ...
            for i in range(xtraj.shape[1]):
                plt.plot(xtraj[:, i], label='x_' + str(i))
